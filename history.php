...
if ( $_GET['type'] == 'del' && $_GET['delid'] != '' )   //刪除配餐紀錄
{
...
/*if ( $_GET['type'] == 'use' && $_GET['useid'] != ''  )  //使用配餐紀錄
{
	$userFood = mysql_fetch_array(mysql_query("SELECT * FROM user_food WHERE id = '" . $_GET['userid'] . "'"));
	if ( $userFood['userid'] != $USER['userid'] )
	{
		showMsg('使用本配餐紀錄失敗!!');
		reDirect(ROOT_URL . '/history.php');
	}else{
		$sql = "INSERT INTO guest_food ()VALUES( ";
		if ( delete_value('user_food', "WHERE id = '" . $_GET['delid'] . "'") )
		{
			showMsg('使用本配餐紀錄成功!!');
			reDirect(ROOT_URL . '/history.php');
		}else{
			showMsg('使用本配餐紀錄失敗，請洽系統管理員!!');
			reDirect(ROOT_URL . '/history.php');
		}
	}
}*/
...
<tr>
	...
	<tr>
		...
		<div class = 'fast_link'>
		<?PHP
		$nowL = array('首頁' => 'index.php', '我的配餐紀錄' => 'history.php');
	...
	<tr>
		<td valign = 'top'>
		<div class = 'title3' style = 'padding-left:50px;padding-top:5px;padding-bottom:5px'>我的配餐紀錄</div>
